...
func GetAllTimeTeams(id int, db *sql.DB) ([]string, error) {
	query := fmt.Sprintf(
		`WITH maxSeason AS (
    	SELECT MAX(season) AS season FROM batting
	),
	current AS (
		SELECT DISTINCT team 
		FROM batting 
		WHERE player_id = %d AND season = (SELECT season FROM maxSeason)
		UNION 
		SELECT DISTINCT team 
		FROM bowling 
		WHERE player_id = %d AND season = (SELECT season FROM maxSeason)
	...
	SELECT team FROM current
	UNION
	SELECT team FROM otherTeams;`, id,
		id,
		id,
		id,
	)
...
func GetAllTimeBattingStats(id int, db *sql.DB) BatStats {
	query := fmt.Sprintf(
		`SELECT 
			COUNT(*) AS innings, 
			SUM(r) AS totalruns, 
...
func GetAllTimeBattingPerformance(id int, db *sql.DB) BatPerformanceStats {
	query := fmt.Sprintf(
		`
...
func GetALlTimeBowlingStats(id int, db *sql.DB) BowlStats {
	query := fmt.Sprintf(
		`
		SELECT 
			COUNT(*) AS innings,
...
func GetAllTimeBowlingPerformance(id int, db *sql.DB) BowlPerformanceStats {
	query := fmt.Sprintf(
		`
...
func GetAllTimePlayerCard(name string) (PlayerCard, error) {
	...
	teams, err := GetAllTimeTeams(playerCard.PlayerID, database.DB)
	...
	playerCard.Batting.CareerStats = GetAllTimeBattingStats(playerCard.PlayerID, database.DB)

	playerCard.CareerHistory, err = GetAllTimeHistory(playerCard.PlayerID, database.DB)

	playerCard.Batting.PerformanceStats = GetAllTimeBattingPerformance(playerCard.PlayerID, database.DB)

	playerCard.Bowling.CareerStats = GetALlTimeBowlingStats(playerCard.PlayerID, database.DB)

	playerCard.Bowling.PerformanceStats = GetAllTimeBowlingPerformance(playerCard.PlayerID, database.DB)
